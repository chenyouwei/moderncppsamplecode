 ...
 *    default constructor, desctructor, copy operations, and move operations.
 *
 ...
 *    declared copy assignment operator, and it's deleted if a move operation is declared.
 *    Generation of the copy operations in classes with an explicitly declared desctructor
 *    is deprecated.
 *
...
namespace item16 {
    ...
    // For a single variable or memory location requiring sychronization,
    // use of a std::atmoic is adequate, but once you get to two or more
    // variables or memory locations that require manipulation as a unit,
    ...
    public:
        int magicValue() const {
            ...
            if (cacheValid) return cacheValid;
            else {
                auto val1 = 1; // expensive computation
                auto val2 = 2;
                cacheValid = val1 + val2;
                cacheValid = true;
                return cachedValue;
            }
        }
...
namespace item17 {
    // Move operations (move constructor and move assignment) are not independent.
    // If you declare either, that prevents compiler from generating the other.


    // The C++11 rules governing the special member functions are thus:
    // # Default contrucotr: Same rule as C++98. Generated only if the class contains
    //   no user-declared constructors.
    //
    // # Desctuctor: Essetially same rules as C++98; sole difference is that destructors
    //   are noexcept by default. As in C++98, virtual only if a base class destructor is
    //   virtual.
    //
    // # Copy constructor: Same runtime behavior as C++98: memberwise copy construction of
    //   non-static data members. Generated only if the class lacks a user-declared copy
    //   constructor. Deleted if the class declares a move operation. Generation of this
    //   function in a class with a user-declared copy assignment operator or destructor is
    //   depracated.
    //
    // # Copy assignment operator: Same runtime behavior as C++98: memberwise copy construction
    //   of non-static data members. Generated only if the class lacks a user-declared copy
    //   constructor. Deleted if the class declares a move operation. Generation of this
    //   function in a class with a user-declared copy constructor or destructor is depracated.
    //
    // # Move constructor and move assignment operator: Each performs memberwise moving of non-static
    //   data members. Generated only if the class contains no use-declared copy operations, move
    //   operations, or desctructor.
    //
