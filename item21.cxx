...
int main() {
    using namespace item21;

    processWidget(std::shared_ptr<Widget>(new Widget),  // potential resource
                  computePriority());                   // leak!

    // At runtime, the arguments for a function must be evaluated before
    // the function can be invoked.

    processWidget(std::make_unique<Widget>(),   // no potential
                  computePriority());           // resource leak

    // create std::initliaizer_list
    auto initList = {10, 20};
